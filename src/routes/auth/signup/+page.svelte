<script>
	...
	$: {
		if (form?.error === "Invalid login credentials") {
			form.error = get(wordList).error.wrongCredentials;
		} else if (form?.error === "Password should be at least 6 characters") {
			form.error = get(wordList).error.toShortPassword;
		} else {
			if (form) {
				fetch("/auth/createUserdata", {
					method: "POST",
					body: JSON.stringify({
						id: form.user.id
					})
